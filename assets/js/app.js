document.addEventListener("DOMContentLoaded", () => {
  ...
  const setupImageFallbacks = () => {
    const fallbackSrc = "assets/images/image-fallback.svg";
  ...
  const toStaticPageUrl = (route = "") => {
    ...
    if (!cleanPath) return hash ? "index.html#" + hash : "index.html";
    if (cleanPath.includes(".") || cleanPath.includes("/")) return hash ? cleanPath + "#" + hash : cleanPath;
    return hash ? cleanPath + ".html#" + hash : cleanPath + ".html";
  };

  const getStaticPartnerLogos = () => ({
    text: normalizePartnerSectionText(),
    items: [
      { title: "تحليل الأعمال", route: "solution-template.html", logo_url: "assets/images/icon-discovery.svg" },
      { title: "تنفيذ ERP", route: "catalog.html", logo_url: "assets/images/icon-erp.svg" },
      { title: "تكاملات API", route: "solution-template.html", logo_url: "assets/images/icon-integrations.svg" },
      { title: "AI Operations", route: "solution-template.html", logo_url: "assets/images/icon-ai.svg" },
    ],
  });
  ...
  const renderPartnerLogos = (track, logos) => {
    ...
    const toCard = (item) => {
      ...
      const link = document.createElement("a");
      link.href = toStaticPageUrl(item.route || "blog-list.html");
  ...
  const setupPartnerLogos = async () => {
    ...
    try {
      const partnerPayload = getStaticPartnerLogos();
  ...
  leadForms.forEach((form) => {
    form.addEventListener("submit", async (event) => {
      ...
      setStatus("جاري تجهيز رسالة الطلب...");
      ...
      try {
        ...
        lines.push("source_page: " + window.location.href);

        const subject = encodeURIComponent("Website request from connect4systems.com");
        const body = encodeURIComponent(lines.join("\n"));
        window.location.href = "mailto:info@connect4systems.com?subject=" + subject + "&body=" + body;

        form.reset();
        setStatus("تم فتح برنامج البريد لإرسال الطلب.", "success");
      } catch (error) {
        const message = error instanceof Error ? error.message : "تعذر تجهيز رسالة الطلب حاليا. تواصل معنا على info@connect4systems.com.";
